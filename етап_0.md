# Етап 0 — Фундамент + SEO-інфраструктура: скрипт повного відтворення

**Як запустити** (Linux Mint, термінал):

```bash
mkdir -p ~/projects && cd ~/projects
nano stage0_foundation.sh        # вставте скрипт нижче, збережіть (Ctrl+O, Ctrl+X)
chmod +x stage0_foundation.sh && ./stage0_foundation.sh
```

Скрипт сам: поставить системні залежності (якщо бракує) → створить venv → `django-admin startproject` → `startapp` × 7 → запише всі файли 1:1 → міграції → суперюзер → i18n-компіляція → Tailwind-збірка → прожене тести.
...
mkdir -p templates/partials static/css static/images locale docs/adr \
...
| # | Перевірка | Як |
|---|---|---|
| 1 | `pytest` — 9 тестів зелені | автоматично в скрипті |
| 2 | `/` → 302 на `/en/` (Accept-Language) | браузер |
| 3 | Перемикач EN/ES у навігації зберігає контекст сторінки | клік ES → футер іспанською |
| 4 | Темна тема без «блимання» при перезавантаженні | F5 на темній |
| 5 | `view-source:/en/` містить `hreflang="es"` + `x-default` + canonical | Ctrl+U |
| 6 | `/sitemap.xml` — альтернативи EN/ES | перегляд XML |
| 7 | Мобільне меню + cookie-банер + WhatsApp FAB | DevTools → mobile |

**Відомі тимчасові обмеження Етапу 0** (закриваються наступними етапами): посилання футера на legal-сторінки та локації ведуть на заглушки (Етап 2/4), секції головної — плейсхолдери (Етап 3), `/booking/` — заглушка (Етап 5). Легасі-редиректи — 302, перемикаються на 301 після появи цільових сторінок.

Скажіть **«далі»** — і я напишу скрипт **Етапу 1: ядро CMS** (`SiteSettings` solo, `Page`/`Section`/`SectionItem`/`MenuLink` з parler, перекладені slug'и, sortable-адмінка, PageView-рушій) — так само, одним sh-скриптом, що продовжує роботу в цій самій директорії проєкту.
```
